...
function createRequestOption(mock, ctx) {
	let protocol = mock && mock.protocol ? mock.protocol : ctx.protocol,
	...
	let buildUrl = function () {
		var arr = [protocol, "://", host, Number.isInteger(port) && port != 80 ? (":" + port) : "",
			path
		];
		if (query) {
			let tmpArray = [];
	...
	// protocol, such as http: https: etc.
	...
	if (ctx.headers) {
		Object.keys(ctx.headers).forEach(key => {
			// handle cache header.
			if (!mock.cache && (key === "if-modified-since" || key === "if-none-match")) {
				return true;
			}
...
module.exports = function (options) {
	...
	const defaultCharset = op.charset || "utf-8";
	const isInExtList = function(ext){
		for(let i=0; i < extList.length; i++){
			let item = extList[i];
			if(item === "all" || item === "*" || item === ext){
				return true;
			}
		}
		return false;
	...
	if(op.ext){
		let tmpList = op.ext;
		if(typeof op.ext === "string"){
			tmpList = [op.ext];
		}
		if(Array.isArray(tmpList)){
			Array.prototype.push.apply(extList, tmpList);
		}
	}
			...
			try {
				...
				if (typeof ret !== "boolean" || ret === true) {
					...
					if (response) {
						...
						if (statusCode >= 200 && statusCode < 300 && isInExtList(ext)) {
							proxy.result = iconv.decode(response.body, charset || defaultCharset);
						} else {
							// return immdiately
